...
class UsersController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if (!$userRecord) {
            $userRecord = User::selectRaw('COUNT(id) AS totalUser')
                ->selectRaw('COUNT(CASE WHEN status = 1 THEN 1 END) AS activeUser')
                ->selectRaw('ROUND(COUNT(CASE WHEN status = 1 THEN 1 END) * 100.0 / COUNT(id), 2) AS activeUserPercentage')
                ->selectRaw('COUNT(CASE WHEN status = 0 THEN 1 END) AS inactiveUser')
                ->selectRaw('ROUND(COUNT(CASE WHEN status = 0 THEN 1 END) * 100.0 / COUNT(id), 2) AS inactiveUserPercentage')
                ->selectRaw('COUNT(CASE WHEN DATE(created_at) = CURDATE() THEN 1 END) AS todayJoined')
                ->selectRaw('ROUND(COUNT(CASE WHEN DATE(created_at) = CURDATE() THEN 1 END) * 100.0 / COUNT(id), 2) AS todayJoinPercentage')
                ->selectRaw('COUNT(CASE WHEN MONTH(created_at) = MONTH(CURDATE()) AND YEAR(created_at) = YEAR(CURDATE()) THEN 1 END) AS thisMonthJoined')
                ->selectRaw('ROUND(COUNT(CASE WHEN MONTH(created_at) = MONTH(CURDATE()) AND YEAR(created_at) = YEAR(CURDATE()) THEN 1 END) * 100.0 / COUNT(id), 2) AS thisMonthJoinPercentage')
    ...
    public function search(Request $request)
    {
        ...
        return DataTables::of($users)
            ...
            })->rawColumns(['action', 'checkbox', 'name', 'balance', 'email-phone', 'status','last-login', 'email_v', 'sms_v'])
            ->make(true);
    }
    ...
    public function userStore(Request $request)
    {
        ...
        try {
            ...
            UserNotificationTempletes::dispatch($response->id);
    ...
    public function userTransactionSearch(Request $request, $id)
    {
        ...
        return DataTables::of($transaction)
            ...
            ->addColumn('amount', function ($item) use($id) {
                if ($item->vendor_id) {
                    $currentTrxType = ($item->vendor_id == $id) ? '+' : '-';
                    $currentStatusClass = ($item->vendor_id == $id) ? 'text-success' : 'text-danger';
                }else{
    ...
    public function userPaymentSearch(Request $request, $id)
    {
        ...
        return DataTables::of($funds)
            ...
            ->addColumn('method', function ($item) use ($id) {
                $received = ($item->user_id != $id) ? '(Received)' : '';
    ...
    public function userPayoutSearch(Request $request, $id)
    {
        ...
        return DataTables::of($payout)
            ...
            ->addColumn('action', function ($item) use ($basicControl) {
                ...
                data-amount=' " . currencyPosition($item->amount_in_base_currency) . "'
    ...
    public function userBookingSearch(Request $request, $id)
    {
        ...
        return DataTables::of($Bookings)
            ...
            ->rawColumns(['action', 'checkbox','booking-id', 'package','status', 'duration', 'date','price'])
            ->make(true);
    }

    public function setPaymentCollection(Request $request)
    {
        $request->validate([
            'user_id' => 'required|integer|exists:users,id',
            'payment_collection' => 'required|boolean',
        ]);
        ...
        $user->payment_collection_system = $request->payment_collection;
        $user->save();
